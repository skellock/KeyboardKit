...
class BaseKeyboardLayoutProviderTests: QuickSpec {
    ...
    override func spec() {
        ...
        describe("layout rows for action rows") {
            
            func result(for rows: KeyboardActionRows) -> [KeyboardLayoutRow] {
                provider.layoutRows(for: rows)
            }
            
            it("is correct for all actions") {
                let row1: [KeyboardAction] = [.backspace, .dictation]
                let row2: [KeyboardAction] = [.space, .command]
                let rows = result(for: [row1, row2])
                expect(rows[0].items[0]).to(equal(expectedItem(for: .backspace)))
                expect(rows[0].items[1]).to(equal(expectedItem(for: .dictation)))
                expect(rows[1].items[0]).to(equal(expectedItem(for: .space)))
                expect(rows[1].items[1]).to(equal(expectedItem(for: .command)))
            }
        }
        
        describe("layout row for actions") {
            
            func result(for actions: KeyboardActions) -> KeyboardLayoutRow {
                provider.layoutRow(for: actions, at: 0)
            }
            
            it("is correct for all actions") {
                let row = result(for: [.backspace, .dictation])
                expect(row.items[0]).to(equal(expectedItem(for: .backspace)))
                expect(row.items[1]).to(equal(expectedItem(for: .dictation)))
            }
        }
